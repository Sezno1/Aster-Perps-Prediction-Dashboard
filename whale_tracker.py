...
class WhaleTracker:
    def __init__(self, db_path: str = "whale_trades.db", whale_threshold_usd: float = 5000):
    ...
    def analyze_trades(self, trades: List[Dict], current_price: float) -> Dict:
        ...
        for trade in trades:
            ...
            if usd_value >= self.whale_threshold:
                whale_trade = {
                    'trade_id': trade.get('id') or trade.get('agg_id'),
    ...
    def _log_whale_trade(self, trade_id: int, timestamp: datetime, price: float,
        ...
        try:
            with self.lock:
                ...
                cursor.execute("""
                    ...
                    (trade_id, timestamp, price, quantity, usd_value, direction, is_buyer_maker)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (trade_id, timestamp, price, quantity, usd_value, direction, is_buyer_maker))
    ...
    def get_recent_whales(self, minutes: int = 60, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT timestamp, price, quantity, usd_value, direction
                FROM whale_trades
                WHERE timestamp >= ?
            ...
            for row in cursor.fetchall():
                whales.append({
                    ...
                    'price': f"${row[1]:.6f}",
                    'quantity': f"{row[2]:,.0f}",
                    'usd_value': f"${row[3]:,.0f}",
                    'action': row[4],
                    'type': 'bullish' if row[4] == 'BUY' else 'bearish'
                })
